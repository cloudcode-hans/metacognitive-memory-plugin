...
export class MemoryStore {
  ...
  private async _init(): Promise<void> {
    ...
    // Ensure meta rows
    this.db.run(
      `INSERT OR IGNORE INTO meta (key, value, updated_at) VALUES ('version', '1.0.0', datetime('now'))`
    );
    this.db.run(
      `INSERT OR IGNORE INTO meta (key, value, updated_at) VALUES ('created_at', ?, datetime('now'))`,
      [nowIso()]
    );
  }
  ...
  private listRows<T>(sql: string, params: unknown[] = []): T[] {
    ...
    while (stmt.step()) {
      const vals = stmt.get();
      const row = {} as T;
      for (let i = 0; i < cols.length; i++) (row as Record<string, unknown>)[cols[i]] = vals[i];
      rows.push(row);
    }
    stmt.free();
    return rows;
  ...
  l5AddFact(p: {
    ...
    const id = uuid();
    this.execute(
      `INSERT INTO knowledge_fact (id, session_id, domain, fact_text, confidence, tags, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        p.sessionId,
  ...
  l6SelfCheck(sessionId: string): {
    ...
    for (const r of errorRows) errorStats[r.error_type] = r.cnt;
    ...
    for (const r of capRows) capCategories[r.category] = r.cnt;
