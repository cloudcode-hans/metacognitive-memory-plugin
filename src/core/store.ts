...
export class MemoryStore {
  ...
  private dictRow(sql: string, params: unknown[] = []): Record<string, unknown> | null {
  ...
  private listRows<T>(sql: string, params: unknown[] = []): T[] {
    if (!this.db) throw new Error("Store not initialized");
    const stmt = this.db.prepare(sql);
  ...
  private storeEmbedding(p: {
    ...
    this.execute(
      `INSERT INTO memory_embeddings (id, session_id, embedding, dimension, source_type, source_id, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [id, p.sessionId, blob, p.embedding.length, p.sourceType, p.sourceId, nowIso()]
    );
    
    try {
      this.execute(
        `INSERT INTO memory_embeddings_idx(rowid, embedding) VALUES ((SELECT rowid FROM memory_embeddings WHERE id = ?), ?)`,
        [id, blob]
      );
    } catch {
    }
  ...
  vectorSearch(p: {
    ...
    sourceType?: string;
    limit?: number;
  }): { sourceId: string; sourceType: string; distance: number }[] {
    if (!this.vecEnabled || !this.db) return [];
    ...
    let sql = `
      ...
      WHERE e.session_id = ?
    `;
    const params: unknown[] = [queryBlob, sessionId];
    ...
    try {
      return this.listRows<{ source_id: string; source_type: string; distance: number }>(sql, params)
        .map(row => ({
  ...
  l0Capture(p: {
    ...
    const id = uuid();
    this.execute(
      `INSERT INTO memory_l0_raw (id, session_id, role, content, tool_name, tool_result, timestamp, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, p.sessionId, p.role, p.content, p.toolName ?? null, p.toolResult ?? null, nowIso(), nowIso()]
    );
    return id;
  }

  l0List(sessionId: string, limit = 50): L0Row[] {
    return this.listRows<L0Row>(
      `SELECT * FROM memory_l0_raw WHERE session_id = ? ORDER BY timestamp DESC LIMIT ?`,
      [sessionId, limit]
    );
  }
  ...
  l0ListAll(limit = 50): L0Row[] {
    return this.listRows<L0Row>(
      `SELECT * FROM memory_l0_raw ORDER BY timestamp DESC LIMIT ?`,
      [limit]
    );
  }

  l1ListAll(limit = 50): L1Row[] {
    return this.listRows<L1Row>(
      `SELECT * FROM memory_l1_extracted ORDER BY created_at DESC LIMIT ?`,
      [limit]
    );
  }
  ...
  private classifyContent(content: string): MemoryType {
    const c = content.toLowerCase();
    if (/喜欢|讨厌|偏好|always|never|prefer|want|hate/.test(c)) return "preference";
    if (/决定|选择|不选|决定用|chosen|will go with|picked/.test(c)) return "decision";
    if (/我叫|名字是|我是|fact:|truth:/.test(c)) return "fact";
    if (/记住|记下|别忘|must|should|need to/.test(c)) return "instruction";
    return "context";
  }

  private dedupHash(content: string): string {
  ...
  l1Extract(sessionId: string, maxMemories = 30): number {
    const l0Rows = this.listRows<L0Row>(
      `SELECT id, content FROM memory_l0_raw
       WHERE session_id = ?
       AND id NOT IN (
       ...
       ORDER BY timestamp
       LIMIT ?`,
      [sessionId, sessionId, maxMemories]
    );

    let count = 0;
    for (const row of l0Rows) {
      const l0Id = row.id as string;
      const content = row.content as string;
      const dedup = this.dedupHash(content);

      const existing = this.dictRow(
        `SELECT 1 FROM memory_l1_extracted WHERE session_id = ? AND dedup_hash = ? LIMIT 1`,
        [sessionId, dedup]
      );
      ...
      this.execute(
        `INSERT INTO memory_l1_extracted
         (id, session_id, memory_text, memory_type, importance, dedup_hash, source_ids, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [memId, sessionId, content, memType, importance, dedup, sourceIdsJson, nowIso(), nowIso()]
      );
      count++;
    }
    return count;
  ...
  l1List(
    ...
    return this.listRows<L1Row>(sql, params);
  }
  ...
  l1AddEmbedding(p: {
    ...
    this.execute(
      `UPDATE memory_l1_extracted SET embedding = ?, updated_at = ? WHERE id = ? AND session_id = ?`,
      [blob, nowIso(), p.memoryId, p.sessionId]
    );
  ...
  l2CreateScene(p: {
    ...
    const id = uuid();
    this.execute(
      `INSERT INTO memory_l2_scenes (id, session_id, scene_type, title, background, decision, result, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, p.sessionId, p.sceneType, p.title, p.background ?? null, p.decision ?? null, p.result ?? null, nowIso()]
    );
    return id;
  }
  ...
  l2ListScenes(sessionId: string, sceneType?: SceneType, limit = 50): L2Row[] {
    ...
    return this.listRows<L2Row>(sql, params);
  }
  ...
  private static readonly MEANINGFUL_CHARS = /[\u4e00-\u9fff]|[a-zA-Z]{2,}/;
  ...
  private extractEntityName(text: string, keyword: string): string {
    ...
    if (hasChinese && candidate.length < 2) return '';
    if (hasEnglish && !hasChinese && candidate.replace(/[^a-zA-Z]/g, '').length < 3) return '';
    ...
    if (hasChinese) {
      ...
      if (nonStopwordChars.length < chars.length * 0.3) return '';
      // Also check if name is purely stopwords
      if (chars.every(c => MemoryStore.ZH_STOPWORDS.has(c) || /[，。！？、：；,\.!?\s]/.test(c))) return '';
    }

    // 4. Must contain at least some meaningful characters
    if (!MemoryStore.MEANINGFUL_CHARS.test(candidate)) return '';

    return candidate;
  }

  l3UpdateGraph(sessionId: string): { entitiesCreated: number } {
    const rows = this.listRows<{ id: string; memory_text: string }>(
      `SELECT id, memory_text FROM memory_l1_extracted
       WHERE session_id = ?
    ...
    for (const row of rows) {
      const text = row.memory_text as string;
      for (const [entType, kws] of Object.entries(KNOWN_ENTITIES)) {
        for (const kw of kws) {
          if (text.toLowerCase().includes(kw.toLowerCase())) {
            const name = this.extractEntityName(text, kw);
            if (name.length < 2) continue;
            const existing = this.listRows<{ id: string; frequency: number }>(
              `SELECT id, frequency FROM kg_entity WHERE session_id = ? AND name = ?`,
              [sessionId, name]
            );
            if (existing.length > 0) {
              this.execute(
                `UPDATE kg_entity SET frequency = frequency + 1, last_seen = ? WHERE id = ?`,
                [nowIso(), existing[0].id]
              );
            } else {
              this.execute(
                `INSERT INTO kg_entity (id, session_id, entity_type, name, last_seen, frequency, created_at)
                 VALUES (?, ?, ?, ?, ?, 1, ?)`,
                [uuid(), sessionId, entType, name, nowIso(), nowIso()]
              );
              entitiesCreated++;
            }
            break;
  ...
  l3ListEntities(sessionId: string, entityType?: EntityType, limit = 50): KGEntityRow[] {
    ...
    return this.listRows<KGEntityRow>(sql, params);
  }
  ...
  l3ListRelations(sessionId: string, fromEntityId?: string, limit = 50): KGRelationRow[] {
    ...
    return this.listRows<KGRelationRow>(sql, fromEntityId ? [sessionId, fromEntityId, limit] : [sessionId, limit]);
  }
  ...
  l3AddEntityEmbedding(p: {
    ...
    this.execute(
      `UPDATE kg_entity SET embedding = ?, updated_at = ? WHERE id = ? AND session_id = ?`,
      [blob, nowIso(), p.entityId, p.sessionId]
    );
  ...
  l4CreateGoal(p: {
    ...
    const id = uuid();
    this.execute(
      `INSERT INTO goal (id, session_id, parent_id, title, description, status, priority, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, ?)`,
      [
        id,
        p.sessionId,
  ...
  l4Tree(sessionId: string): Record<string, unknown>[] {
    return this.listRows<Record<string, unknown>>(
      ...
      SELECT id, parent_id, title, status, priority, depth FROM goal_tree ORDER BY depth, priority`,
      [sessionId, sessionId]
    );
  }
  ...
  l5AddFact(p: {
    ...
    this.execute(
      `INSERT INTO knowledge_fact (id, session_id, domain, fact_text, embedding, confidence, tags, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        p.sessionId,
  ...
  l5Search(sessionId: string, query: string, limit = 20): KnowledgeFactRow[] {
    const q = `%${query}%`;
    return this.listRows<KnowledgeFactRow>(
      `SELECT id, domain, fact_text, confidence, verified, created_at
       FROM knowledge_fact
  ...
  l5VectorSearch(p: {
    ...
    const facts = this.listRows<{ id: string; fact_text: string; confidence: number }>(
      `SELECT id, fact_text, confidence FROM knowledge_fact WHERE id IN (${placeholders})`,
      factIds
    );
  ...
  l6LogError(p: {
    ...
    const id = uuid();
    this.execute(
      `INSERT INTO error_log (id, session_id, error_type, summary, detail, stack_trace, context, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        p.sessionId,
  ...
  l6SelfCheck(sessionId: string): {
    ...
    const errorRows = this.listRows<{ error_type: string; cnt: number }>(
      `SELECT error_type, COUNT(*) as cnt FROM error_log WHERE session_id = ? AND resolved = 0 GROUP BY error_type`,
      [sessionId]
    );
    ...
    const capRows = this.listRows<{ category: string; cnt: number }>(
      `SELECT category, COUNT(*) as cnt FROM self_model WHERE session_id = ? GROUP BY category`,
      [sessionId]
    );
    const capCategories: Record<string, number> = {};
    ...
    const verifiedCount = this.listRows<{ cnt: number }>(
      `SELECT COUNT(*) as cnt FROM self_model WHERE session_id = ? AND verified = 1`,
      [sessionId]
    )[0]?.cnt ?? 0;

    const recentErrors = this.listRows<ErrorLogRow>(
      `SELECT id, error_type, summary, created_at FROM error_log WHERE session_id = ? ORDER BY created_at DESC LIMIT 5`,
      [sessionId]
    );
  ...
  purgeByFrequency(minFrequency: number, maxDays: number, sessionId?: string): { totalDeleted: number; minFrequency: number; maxDays: number; sessionId: string | null } {
    ...
    const entitiesToDelete = this.listRows<{ id: string }>(baseQuery, params);
  ...
  smartPurge(config: {
    ...
    const l1Rows = this.listRows<{ id: string; importance: number }>(
      sessionId
        ? `SELECT id, importance FROM memory_l1_extracted WHERE session_id = ? AND created_at < datetime('now', '-${defaultRetentionDays} days')`
        : `SELECT id, importance FROM memory_l1_extracted WHERE created_at < datetime('now', '-${defaultRetentionDays} days')`,
      sessionId ? [sessionId] : []
    );
    ...
    for (const row of l1Rows) {
      ...
      const ageCheck = this.listRows(
        `SELECT 1 FROM memory_l1_extracted WHERE id = ? AND created_at < datetime('now', '-${retentionDays} days')`,
        [row.id]
      );
    ...
    const entityRows = this.listRows<{ id: string; frequency: number }>(
      sessionId
        ? `SELECT id, frequency FROM kg_entity WHERE session_id = ?`
        : `SELECT id, frequency FROM kg_entity`,
      sessionId ? [sessionId] : []
    );
    
    for (const row of entityRows) {
      let retentionDays: number;
      if (row.frequency >= highFreqThreshold) retentionDays = highFreqRetentionDays;
      else if (row.frequency >= mediumFreqThreshold) retentionDays = mediumFreqRetentionDays;
      else retentionDays = lowFreqRetentionDays;
      
      const ageCheck = this.listRows(
        `SELECT 1 FROM kg_entity WHERE id = ? AND last_seen < datetime('now', '-${retentionDays} days')`,
        [row.id]
      );
  ...
  getStorageStats(): { stats: Record<string, number>; totalRows: number; vectorEnabled: boolean } {
    ...
    for (const table of tables) {
      try {
        const count = this.listRows<{ cnt: number }>(`SELECT COUNT(*) as cnt FROM ${table}`)[0]?.cnt ?? 0;
        stats[table] = count;
        totalRows += count;
      } catch {
        stats[table] = 0;
  ...
  getVersion(): string {
    ...
    return row?.value as string || "1.0.0";
  }
  ...
  l1GetSessionState(sessionId: string, stateKey: string): unknown | null {
    const row = this.dictRow(
      `SELECT state_value FROM session_state WHERE session_id = ? AND state_key = ? AND (locked_until IS NULL OR locked_until > datetime('now'))`,
      [sessionId, stateKey]
    );
    if (!row) return null;
    try {
      return JSON.parse(row.state_value as string);
    } catch {
  ...
  l1ListSessionStates(sessionId: string): Array<{ key: string; value: unknown; updatedAt: string }> {
    const rows = this.listRows<{ state_key: string; state_value: string; updated_at: string }>(
      `SELECT state_key, state_value, updated_at FROM session_state WHERE session_id = ?`,
      [sessionId]
    );
    return rows.map(row => ({
      key: row.state_key,
  ...
  l1AcquireLock(p: { sessionId: string; lockKey: string; timeoutSeconds?: number }): boolean {
    ...
    const existing = this.dictRow(
      `SELECT locked_until FROM session_state WHERE session_id = ? AND state_key = ?`,
      [p.sessionId, p.lockKey]
    );
    ...
    this.execute(
      `INSERT OR REPLACE INTO session_state 
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [uuid(), p.sessionId, p.lockKey, 'locked', 'lock', lockedUntil, nowIso(), nowIso()]
    );
    return true;
  }
  ...
  l1ReleaseLock(sessionId: string, lockKey: string): void {
    this.execute(
      `DELETE FROM session_state WHERE session_id = ? AND state_key = ? AND state_type = 'lock'`,
      [sessionId, lockKey]
    );
  }
  ...
  l3HybridSearch(p: {
    ...
    let vecResults: Array<{ sourceId: string; sourceType: string; distance: number }> = [];
    ...
    const fusedResults: Array<{ sourceId: string; sourceType: string; score: number; content: string }> = [];
  ...
  l3IndexSemanticContent(p: {
    ...
    try {
      this.execute(
        `INSERT INTO semantic_fts (session_id, memory_text, entity_name, fact_text)
         VALUES (?, ?, ?, ?)`,
        [p.sessionId, p.memoryText || null, p.entityName || null, p.factText || null]
      );
    } catch {
    }
  }
  ...
  fileIndexSearch(p: {
    ...
    const params: unknown[] = [p.sessionId, q, q];
